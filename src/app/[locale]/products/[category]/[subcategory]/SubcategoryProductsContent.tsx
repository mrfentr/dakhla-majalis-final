...
export default function SubcategoryProductsContent({ categorySlug, subcategorySlug }: Props) {
  ...
  const dbProducts = useMemo(() => {
    ...
    return allProducts.filter(
      p => p.category === categorySlug && p.subcategory === subcategorySlug && p.status === 'active'
    );
  }, [allProducts, categorySlug, subcategorySlug]);

  // Find parent category and subcategory for display names
  const parentCategory = (dbCategories ?? []).find(c => c.slug === categorySlug && !c.parentCategoryId);
  const subcategory = (dbCategories ?? []).find(c => c.slug === subcategorySlug && c.parentCategoryId);

  const parentCategoryName = parentCategory ? getLocalizedField(parentCategory.name, locale) : categorySlug;
  const subcategoryName = subcategory ? getLocalizedField(subcategory.name, locale) : subcategorySlug;

  // Build category ID → slug map for fabric variant matching
  ...
  const products: Product[] = useMemo(() => {
    ...
    const fabricVariantIdsInThisSubcategory = new Set(
    ...
    const productsReplacedByFabricVariant = new Set(
      dbProducts
        .filter(p => p.fabricVariantId && fabricVariantIdsInThisSubcategory.has(p.fabricVariantId))
        .map(p => p._id)
    );

    const mappedProducts: Product[] = dbProducts
      .filter(product => !product.isMandatory && !productsReplacedByFabricVariant.has(product._id))
      .map(product => {
    ...
    // Merge fabric variants that belong to this subcategory
  ...
  }, [dbProducts, activeFabricVariants, categoryIdToSlug, subcategorySlug, categorySlug, t, locale]);
      ...
      <section className="w-full bg-white border-b border-neutral-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 lg:py-12">
          ...
          <nav className="flex items-center gap-2 text-sm text-neutral-400 mb-4 sm:mb-6">
            ...
            <button
              onClick={() => router.push(`/products/${categorySlug}`)}
              className="hover:text-[#B85C38] transition-colors"
              style={{ fontFamily: "'Noto Naskh Arabic', serif" }}
            >
              {parentCategoryName}
            ...
            <ChevronRight size={14} className={isRTL ? 'rotate-180' : ''} />
            <span className="text-[#1A1A1A] font-semibold">{subcategoryName}</span>
          </nav>
          ...
          <p className="text-neutral-500 text-sm sm:text-base max-w-xl leading-relaxed">
            {t('subtitleWithCategory', { category: subcategoryName })}
          </p>
        </div>
      </section>
